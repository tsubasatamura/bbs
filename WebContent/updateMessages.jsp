...
function passCheck(val){
	...
	if(!(pw==val)){
		...
		return false;
	}else if(document.frm1.newpass.value==""){
		alert("新編集用パスワードを入力してください。");
		document.frm1.pass.focus();
		return false;
	}
}
...
while(rs.next()){
	...
	if(messages!=null){
		messages=messages.replace("<br/>",System.lineSeparator());
	}
%>

<form id="frm1" name="frm1" method="post" action="PostMessages" onsubmit="return passCheck('<%= pass%>')">
<table>
<tr><th>タイトル</th><td colspan="2"><input type="text" name="title" value="<%=rs.getString("bbs_title") %>" maxlength="20" size="50"/></td></tr>
<tr><th>名前</th><td colspan="2"><input type="text" name="name" value="<%=rs.getString("bbs_name") %>" maxlength="15" size="30"/></td></tr>
<tr><th>新編集用パスワード</th><td colspan="2"><input type="password" name="newpass"/></td></tr>
<tr><th>内容</th><td colspan="2" width="80%"><textarea name="messages" cols="70" rows="20"><%= messages%></textarea></td></tr>
<tr><th>編集用パスワード</th><td><input type="password" name="pass"/></td>
	<td id="button"><input type="submit" value="変更"/></td>
</tr>
